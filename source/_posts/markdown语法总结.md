...
在Typora编辑器里，非源码模式可以实现实时渲染，编写文本后直接就可以看到导出后的效果。本人实际体验后发现，在Typora里可以直接一个enter实现换段落，文字超出一行限制后会自动换行，若要实现主动换行，可以切换到源代码模式。
在源代码模式里，一个enter就是换行，两个enter就是换段落。也可以非源码模式使用shift+enter实现强制换行（后面介绍的列表，每一项内单独换行也可以用这种方法）。
...
在一级列表里嵌套二级列表的方式：在一级列表的某一项后换行，回车删除自动添加的一级序号，再用“数字 + . + 空格”或者“* + 空格”，即可实现该项下创建二级有序或无序列表（后面介绍的复选框列表列表也可以这样嵌套）。

若要跳出二级列表循环，重新创建上一级列表的下一项，则连续按下三次enter即可（连续两次是消除二级列表格式，直接在一级列表的该项下编写普通文本，编写文本后需要再连续按两次enter才能跳出一级列表的该项，新建一级列表的下一项）

示例：

1. 一级列表第一项（然后enter+回车+“1 + . + 空格”）

   1. 二级列表（直接enter）
   2. 二级列表（两次enter）

   一级列表第一项下编写普通文本。（此处再按两次enter）
...
像这样的勾选框列表如何实现呢？（勾选状态只能在编辑器更改，阅读器或网页里是无法勾选的）
...
像这样给一段文本加注释角标，鼠标停留时显示注释（若有网页链接也可以直接点击跳转）的方式如下：
